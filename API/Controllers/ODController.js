  ...
  OrderDetails.findOne({'orderid' : orderid}, function(err, orderExists) {
    if (err) {
      ...
      return res.json({
        ...
        "fulfillmentMessages":[{"text":"Something went wrong!"}],
        "source": 'order info 1'
      });
    }
    if (orderExists) {
      ...
      return res.json({
        "fulfillmentText": [{"text":"Your order is scheduled for " + [orderExists.deliverydate] + ". When would you like to reschedule your order for?"}],
        "fulfillmentMessages": [{"text":[orderExists.deliverydate] + ". When would you like to reschedule your order for?"}],
        "source": "order info 2"
      });
    } else {
      ...
      return res.json({
        ...
        "fulfillmentMessages": [{"text":"Sorry, we couldn't find your order"}],
        "source": "order info 3"
      });
    }
  });
